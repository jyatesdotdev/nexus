...
async def test_weather_agent_success(mock_context, mock_event_queue):
    ...
    assert "The current weather in **Berlin** is Sunny with a temperature of 68°F (20°C)." in result_msg.parts[0].root.text
...
async def test_weather_agent_api_error(mock_context, mock_event_queue):
    ...
    result_msg = mock_event_queue.enqueue_event.call_args_list[1][0][0]
    assert "Could not retrieve weather for Atlantis. The service returned status 404." in result_msg.parts[0].root.text
...
async def test_weather_agent_parse_error(mock_context, mock_event_queue):
    ...
    result_msg = mock_event_queue.enqueue_event.call_args_list[1][0][0]
    assert "Could not parse weather data for ErrorCity." in result_msg.parts[0].root.text
...
async def test_weather_agent_network_error(mock_context, mock_event_queue):
    ...
    respx.get("https://wttr.in/NetworkErrorCity?format=j1").mock(side_effect=httpx.RequestError("Connection failed", request=MagicMock()))
    
    executor = WeatherAgentExecutor()
    await executor.execute(mock_context, mock_event_queue)
    
    result_msg = mock_event_queue.enqueue_event.call_args_list[1][0][0]
    assert "A network error occurred while fetching the weather: Connection failed" in result_msg.parts[0].root.text
