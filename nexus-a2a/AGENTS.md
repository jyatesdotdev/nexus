...
- `server.py` — the entire service. Contains:
  ...
  - Endpoints served: agent card at `GET /.well-known/agent-card.json`, JSON-RPC at `POST /` (the SDK default — there is no `/rpc` route despite what README.md says), health at `GET /health`.
- `requirements.txt` — runtime + dev deps in one file, including `-e ../nexus-common` (a relative path: `pip install -r requirements.txt` must be run from this repo's root with the sibling checkout present). WARNING: it specifies `a2a-sdk>=0.3.25`, but the code only works with the 0.3.x line; a fresh resolve pulls a2a-sdk 1.x, where `a2a.server.apps` no longer imports (even with the `[http-server]` extra) and `server.py` fails at import. Install `a2a-sdk==0.3.25` if you hit `ModuleNotFoundError: No module named 'a2a.server.apps'`.
...
- `README.md` — human-facing architecture explanation. Mostly accurate, except it claims the JSON-RPC endpoint is `POST /rpc`; the actual endpoint is `POST /`. It also still refers readers to `GEMINI.md`, which has been replaced by this file.
...
- `venv/` (untracked) — a stale virtualenv whose script shebangs point at the old `/Users/jyates/Repositories/nexus-weather/` path and which is missing `nexus_common`/opentelemetry. Do not trust it for running the server; it does still contain a working `a2a-sdk 0.3.25`. Prefer recreating it.
- `.mypy_cache/`, `.ruff_cache/`, `tests/__pycache__/` — tool caches, ignore.

## Subdirectories

- `tests/` — pytest suite for `server.py`. See `tests/AGENTS.md`. Note: 2 of 5 tests currently fail because they assert error-message text that `server.py` does not produce.
...
pip install 'a2a-sdk[http-server]==0.3.25'   # if imports fail with a2a-sdk 1.x
