...
| `nexus-common/` | Shared Python library (`nexus_common` package, ships `py.typed`): identical `/health` bootstrap, OTel tracing, Prometheus metrics, and mock-JWT identity for every Python service. A uv-workspace source dependency of the three services locally; Docker installs it editable (`pip install -e ../nexus-common`) and volume-mounts it into containers. |
...
| `nexus-stack/` | Deployment hub: docker-compose for the four app services, the cross-service Makefile (`make doctor/up/demo/test/lint/verify-all`), `.env` secrets (template: `.env.example`), workspace-wide Semgrep rules. Also `templates/` — scaffold source for `make new-agent NAME=<name>`, which generates a complete new A2A sub-agent service with passing tests. |
...
Local Python environments are managed by a single uv workspace rooted here (since 2026-07-04; there are no per-service venvs):

```bash
uv sync                                                  # once: shared .venv + uv.lock at this level
uv run --project nexus-orchestrator pytest nexus-orchestrator/tests/   # or cd into a service and `uv run pytest`
uv run ruff check nexus-a2a nexus-common nexus-mcp nexus-orchestrator
```

Docker images and CI deliberately do NOT use uv — they pip-install each service's requirements.txt, which is a manual mirror of that service's pyproject deps (keep both in sync; see the header comment in each requirements.txt).
...
- `AGENTS.md` — this file (agent-facing map).
- `README.md` — the human-facing front door: architecture diagrams, quickstart, guided tour, learning-objective table, and the "lessons learned the hard way" section. Keep it in sync with reality like any other doc here.
- `pyproject.toml` — uv workspace root: members are the four Python projects (nexus-integration is deliberately excluded — its tests run inside the orchestrator container).
- `uv.lock` — committed lockfile for the workspace; regenerated by `uv sync`/`uv lock`.
- `.gitignore` — workspace-level ignores (venvs incl. the root `.venv/`, node_modules, caches, `.env`, `*.db`, IDE dirs). `uv.lock` is tracked, not ignored. Service directories layer their own `.gitignore` on top.
