...
@pytest.mark.asyncio
async def test_ollama_adapter_generate_non_streaming():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        assert responses[0].partial == False
...
@pytest.mark.asyncio
async def test_ollama_adapter_generate_streaming():
    adapter = OllamaAdapter(model="ollama/llama3")
    ...
    with patch("httpx.AsyncClient") as mock_client_cls:
        ...
        assert responses[0].partial == True
        assert responses[1].content.parts[0].text == " world"
        assert responses[1].partial == False
