...
SQLAlchemy is an explicit dependency in both `pyproject.toml` and `requirements.txt` (`sqlalchemy[asyncio]>=2.0.0`), listed alongside `asyncpg` and `pgvector`. Do NOT treat it as transitive via `google-adk` — see the pyproject.toml comment: adk 2.x dropped that transitive path and the container broke, so it must stay pinned directly in both mirrored dep files.
