...
class ExtractedEntities(BaseModel):
    ...
    status: str = Field(..., description="Operation status")
    extractions: List[Dict[str, str]] = Field(
        ..., description="List of extracted entities and their types"
    )
...
class BashOutput(BaseModel):
    ...
    output: Optional[str] = Field(
        default=None, description="Standard output from the command"
    )
...
class OrchestratorTool(Protocol):
    ...
    def __call__(self, *args: str, **kwargs: str) -> BaseModel: ...
...
def get_sensor_reading(sensor_id: str) -> SensorReading:
    ...
    return SensorReading(
        status="success", sensor_id=sensor_id, temperature=22.5, humidity=45.0
    )
...
def fetch_ynab_budget(category: str) -> BudgetBalance:
    ...
    return BudgetBalance(
        status="success", category=category, balance=150.00, currency="USD"
    )
...
def extract_entities_with_grounding(text: str, entity_type: str) -> ExtractedEntities:
    ...
    return ExtractedEntities(
        status="success", extractions=[{"text": "Mock", "type": entity_type}]
    )
