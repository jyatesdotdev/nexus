...
from .adapters import bedrock_adapter
...
class AgentFactory(Generic[T]):
    ...
    def create_agent(self, name: str, instruction: str, description: str = None, tools: List[Union[Callable, OrchestratorTool, McpToolset]] = None) -> T:
        return Agent(
            model=self.model,
            name=name,
...
def initialize_agents():
    ...
    sub_agents.append(factory.create_agent(
        name='system_agent',
...
@asynccontextmanager
async def get_runner(agent: Agent):
    """
    ...
    if not await runner.session_service.get_session(app_name=APP_NAME, user_id=DEFAULT_USER_ID, session_id=DEFAULT_SESSION_ID):
        await runner.session_service.create_session(app_name=APP_NAME, user_id=DEFAULT_USER_ID, session_id=DEFAULT_SESSION_ID)
    try:
        yield runner
    finally:
...
async def process_prompt(runner: InMemoryRunner, prompt_text: str):
...
async def run_chat_loop(initial_prompt: Optional[str] = None):
...
class SimpleAgentLoader(BaseAgentLoader):
    ...
    def load_agent(self, app_name: str):
        if app_name == APP_NAME: return self.root_agent
        raise NotFoundError(f"App {app_name} not found")
    def list_agents(self): return [APP_NAME]
    def list_agents_detailed(self):
        return [{"name": APP_NAME, "root_agent_name": "root_agent", "description": "Orchestrator", "language": "python"}]

# Initialize a global root_agent for consistency across commands and tests
root_agent = initialize_agents()

def create_app_instance(agent: Agent):
    ...
    async def get_system_status():
        status = {"orchestrator": "Online", "mcp_servers": {}, "a2a_agents": {}}
        ...
        async with httpx.AsyncClient(timeout=2.0) as client:
            ...
            for i, url in enumerate(MCP_SERVER_URLS):
                try:
                    ...
                    status["mcp_servers"][f"mcp_{i}"] = "Online" if res.status_code == 200 else "Offline"
                except:
            ...
            for i, url in enumerate(A2A_AGENT_URLS):
                try:
                    ...
                    status["a2a_agents"][f"a2a_{i}"] = "Online" if res.status_code == 200 else "Offline"
                except:
