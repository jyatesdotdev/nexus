...
# WHY: json.loads() converts a JSON string (from a network response) 
# into a Python dictionary that we can easily work with.
import json
import os
# HTTPX: A next-generation HTTP client for Python.
# WHY: It fully supports asynchronous requests, which is essential for 
...
# WHY: It allows the ADK to discover and use new adapters dynamically without 
...
class OllamaAdapter(BaseLlm):
    ...
    def supported_models(cls) -> List[str]:
        ...
        # WHY: This tells the ADK that any model string starting with 'ollama/' 
    ...
    async def generate_content_async(
        ...
        # WHY: The ADK uses 'ollama/llama3' to identify this adapter, but 
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            if not stream:
                ...
                async with client.stream("POST", f"{self.base_url}/api/chat", json=payload) as resp:
                    ...
                    # WHY: Ollama streams responses as a series of JSON objects, one per line.
                    async for line in resp.aiter_lines():
                        ...
                        if "message" in chunk and "content" in chunk["message"]:
                            yield LlmResponse(
                                ...
                                content=types.Content(
                                    role="model",
                                    parts=[types.Part(text=chunk["message"]["content"])]
                                )
                            )

# AUTOMATIC REGISTRATION: This module-level code runs exactly once when 
# the module is first imported.
# WHY: This "registers" the adapter so the rest of the app can use it immediately.
