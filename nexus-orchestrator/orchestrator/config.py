...
# WHY: They are values that are set once and not changed during program execution.
# Using uppercase makes them easily identifiable throughout the project.

# OS.GETENV: Reads an environment variable. 
# HOW: os.getenv("VARIABLE_NAME", "default_value").
# WHY: This allows configuration to be external to the code. If the variable 
...
MCP_SERVER_URLS = get_env_list("MCP_SERVER_URLS", os.getenv("MCP_SERVER_URL", "http://mcp-server:8000/sse"))
A2A_AGENT_URLS = get_env_list("A2A_AGENT_URLS", os.getenv("A2A_AGENT_URL", "http://a2a-agent:8001/.well-known/agent-card.json"))
...
def validate_config():
    ...
    if not os.getenv("GEMINI_API_KEY") and not os.getenv("GOOGLE_API_KEY"):
        ...
        # WHY: Better to crash immediately with a clear 'ValueError' than to 
