...
- `agent_registry.py` — The `AgentRegistry` class. All state is a single class-level dict `_agents: Dict[str, Callable]` mapping agent name -> zero-arg factory. Key behaviors and invariants:
  ...
  - `load_agents_from_module(package_name)` imports (or, if already in `sys.modules`, RELOADS) the package and every module inside it via `pkgutil.walk_packages`. The reload is deliberate: after `clear()`, decorators must re-execute to repopulate the registry. Consequence: module-level code in `orchestrator/agents/*` may run multiple times per process — keep those modules side-effect-free apart from registration.
